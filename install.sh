...
print_header() {
    echo -e "${BLUE}duckflow-mcp installer${NC}"
    echo "======================"
    echo ""
    echo "This script installs the duckflow-mcp MCP server from the current checkout:"
    echo "  $REPO_SOURCE"
    echo ""
    echo "It will ask which clients you use, wire duckflow-mcp into their MCP config,"
...
PY
    ...
    success "Merged duckflow instructions into $target_path"
    record_path "$target_path"
}
...
if not isinstance(servers, dict):
    servers = {}
servers["duckflow-mcp"] = {
    "type": "stdio",
    "command": "uvx",
    "args": ["--from", repo_source, "duckflow-mcp"],
}
...
copilot_skill_target() {
    ...
    if [ "$INSTALL_SCOPE" = "user" ]; then
        echo "$vscode_user_dir/skills/duckflow/SKILL.md"
    else
        echo "$PROJECT_DIR/.github/skills/duckflow/SKILL.md"
    fi
}
...
copilot_prompt_target() {
    ...
    if [ "$INSTALL_SCOPE" = "user" ]; then
        echo "$vscode_user_dir/prompts/duckflow.prompt.md"
    else
        echo "$PROJECT_DIR/.github/prompts/duckflow.prompt.md"
    fi
}
...
install_copilot() {
    ...
    merge_markdown_block \
        ...
        "<!-- duckflow-mcp:end -->"
    copy_with_backup \
        "$SCRIPT_DIR/templates/agent-setup/copilot/skills/duckflow/SKILL.md" \
        "$skill_target"
    copy_with_backup \
        "$SCRIPT_DIR/templates/agent-setup/copilot/prompts/duckflow.prompt.md" \
        "$prompt_target"
    echo ""
}
...
print_plan() {
    ...
    echo "  uvx --from $REPO_SOURCE duckflow-mcp"
    echo ""
}
...
print_summary() {
    ...
    echo "If you want clients to install duckflow-mcp from the published GitHub URL instead"
    echo "of this local checkout, use the manual commands in README.md and replace the"
    echo "local --from path with git+https://github.com/warnes-innovations/duckflow."
}
